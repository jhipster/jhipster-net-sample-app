...
namespace JHipsterNetSampleApplication.Test.Web.Identity {
    ...
    public class AccountAnonymousControllerIntTest {
        public AccountAnonymousControllerIntTest()
        {
        ...
        public async Task TestActivateAccount()
        {
            ...
            var response = await client.GetAsync($"/api/account/activate?email={user.Email}&token={token}");
        ...
        {
            var client = _factory.CreateClient();
            var response = await client.GetAsync("/api/account/activate?email=wrongemail&token=wrongActivationKey");
