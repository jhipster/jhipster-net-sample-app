...
namespace JHipsterNetSampleApplication.Test.Web.Identity {
    ...
    public class AccountFixture {
        ...
        public NhipsterWebApplicationFactory<JHipsterStartup> Factory { get; set; } = new NhipsterWebApplicationFactory<JHipsterStartup>();
        ...
        public async Task Initialize()
        {
            ...
            var userManager = Factory.GetRequiredService<UserManager<User>>();
            var passwordHasher = Factory.GetRequiredService<IPasswordHasher<User>>();
    ...
    public class AccountAuthenticatedControllerIntTest : IClassFixture<AccountFixture> {
        AccountFixture fixture;

        public AccountAuthenticatedControllerIntTest(AccountFixture fixture)
        {
        ...
        {
            var userManager = this.fixture.Factory.GetRequiredService<UserManager<User>>();
            var passwordHasher = this.fixture.Factory.GetRequiredService<IPasswordHasher<User>>();

            var response = await this.fixture.Client.PostAsync("/api/account/change-password", TestUtil.ToJsonContent(
            ...
            var updatedUser = await userManager.FindByNameAsync("activate-account");

            //TODO FIX database refresh to prevent the usage of context/Reload
            updatedUser = Fixme.ReloadUser(this.fixture.Factory, updatedUser);
            passwordHasher.VerifyHashedPassword(updatedUser, updatedUser.PasswordHash, "new password")
                .Should()
